...
Настройки языка находятся в `[Localization]`, а все изменяемые строки интерфейса — в секциях `[Strings.ru]` и `[Strings.en]`. INI версий 1–5 автоматически мигрирует в версию 6 с резервной копией; пользовательские переводы сохраняются.
...
```text
artifacts\CmdsManager-portable-0.5.1-win-x64.zip
```
