...
- адаптивный Auto-режим вывода для смешанных строк UTF-8/OEM и явный выбор UTF-8, OEM Windows, Windows-1251 или UTF-16 LE;
...
Настройки языка находятся в `[Localization]`, а все изменяемые строки интерфейса — в секциях `[Strings.ru]` и `[Strings.en]`. INI версий 1 и 2 автоматически мигрирует в версию 3 с резервной копией; пользовательские переводы сохраняются.

Для вывода обычно подходит `OutputEncoding=Auto`: каждая строка сначала строго проверяется как UTF-8, а при недопустимой UTF-8-последовательности декодируется в системной OEM-кодировке Windows. Поэтому в одной вкладке корректно сосуществуют OEM-вывод CMD и UTF-8-вывод Node.js/PowerShell, включая кириллицу и китайские символы. Для принудительного режима выберите конкретную кодировку в редакторе записи.
...
```text
artifacts\CmdsManager-portable-0.3.0-win-x64.zip
```
