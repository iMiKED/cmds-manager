...
- пульсирующий цветной индикатор выполняющегося скрипта и статус на консольной вкладке;
- отдельная вкладка консоли для каждого управляемого экземпляра запуска;
- русский вывод с Auto/OEM и явным выбором UTF-8, Windows-1251 или UTF-16 LE;
- настраиваемый шрифт консоли;
...
Настройки языка находятся в `[Localization]`, а все изменяемые строки интерфейса — в секциях `[Strings.ru]` и `[Strings.en]`. Старый INI версии 1 автоматически мигрирует в версию 2 с резервной копией.

Для вывода обычно подходит `OutputEncoding=Auto`: CmdsManager использует системную OEM-кодировку Windows. Если скрипт выполняет `chcp 65001` или сам пишет UTF-8, выберите `Utf8` в редакторе записи.

## Дочерний управляемый запуск

Каждый запуск через CmdsManager получает отдельную вкладку. Из родительского скрипта можно запустить зарегистрированную запись в уже работающем менеджере по имени или GUID:

```bat
"C:\Path\To\CmdsManager.exe" --run "Имя дочернего скрипта"
```

Такой дочерний запуск имеет собственный PID, Job Object и вкладку. Команды `call child.bat` и `& child.ps1` выполняются внутри текущего интерпретатора и используют его общий stdout, поэтому остаются в родительской вкладке.
...
```text
artifacts\CmdsManager-portable-0.2.1-win-x64.zip
```
