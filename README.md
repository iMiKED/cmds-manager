...
- изменение высоты области консолей splitter-ом с сохранением высоты в INI; область можно развернуть так, чтобы в прокручиваемом списке осталась одна строка;
...
Настройки языка находятся в `[Localization]`, а все изменяемые строки интерфейса — в секциях `[Strings.ru]` и `[Strings.en]`. INI версий 1–4 автоматически мигрирует в версию 5 с резервной копией; пользовательские переводы сохраняются.

Высота нижней области хранится в `ConsolePaneHeight`. Параметры внешнего вида используют цвета `#RRGGBB` и непрозрачность от `0` до `100`. Для встроенного `RichTextBox` прозрачность фона визуально смешивается с системным фоном окна; WinForms не поддерживает настоящую поканальную прозрачность нативного RichEdit-контрола.
...
CmdsManager хранит ограниченную историю сырых байтов вкладки. Если источник определён неверно, щёлкните вкладку правой кнопкой мыши и выберите кодировку: уже показанный текст будет декодирован заново. Там же можно выбрать отдельный шрифт и перенос слов, скопировать выделение либо сохранить выделение/всю вкладку в UTF-8-файл, отделить вкладку, открыть её на весь экран или развернуть всю нижнюю область.

Detach переносит уже существующий контрол вывода между главным и отдельным окном: PID, процесс, Job Object, сырая история, выделение и поток новых строк остаются прежними. Это окно перехваченного вывода CmdsManager, а не новый интерактивный `conhost`. Интерактивный stdin и полноценная терминальная эмуляция требуют запуска процесса через ConPTY с самого начала; добавить ConPTY к уже запущенному процессу с перенаправленными stdout/stderr без перезапуска нельзя.
...
```text
artifacts\CmdsManager-portable-0.5.0-win-x64.zip
```
