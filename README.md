...
- современная компактная таблица, toolbar и контролы окна «Настройки» в стиле Fluent Compact с системной, светлой и тёмной темами без внешних UI-зависимостей;
...
Настройки языка находятся в `[Localization]`, а все изменяемые строки интерфейса — в секциях `[Strings.ru]` и `[Strings.en]`. INI версий 1–6 автоматически мигрирует в версию 7 с резервной копией; пользовательские переводы сохраняются.
...
```text
artifacts\CmdsManager-portable-0.6.3-win-x64.zip
```
